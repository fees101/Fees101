...
export function FeeTrendChart({ fees }: { fees: FeeTrendData[] }) {
  ...
  // Focus one fee → billed vs collected for it.
  if (focus !== '__all__') {
  ...
  // All fees → one line per fee (billed), top 10 by lifetime billing.
...
export function PotentialBreakdown({
  grossPotential, discountTotal, billed, outstanding, collected,
}: {
  ...
  return (
    <Card
      ...
      subtitle={`Of ${naira(grossPotential)} potential, ${collectedPct}% collected · ${foregonePct}% given as discounts`}
    >
      ...
      <div className="flex h-8 w-full rounded-lg overflow-hidden bg-gray-100">
        {segs.map(s => (
          <div key={s.label} style={{ width: `${(s.value / total) * 100}%`, background: s.color }}
            title={`${s.label}: ${naira(s.value)}`} />
        ))}
      </div>

      {/* Stat rows */}
      <div className="mt-5 space-y-3">
        <StatRow color={GREY} label="Potential (before discounts)" value={naira(grossPotential)} bold />
        <StatRow color={RED} label="Given away as discounts" value={`− ${naira(discountTotal)}`} />
        <StatRow color={NAVY} label="Billed to families" value={naira(billed)} bold />
        <StatRow color={AMBER} label="Still outstanding" value={`− ${naira(outstanding)}`} />
        <StatRow color={MINT} label="Actually collected" value={naira(collected)} bold />
      </div>
    </Card>
  )
}
...
// =====================================================================
export function RevenueMix({ fees }: { fees: { name: string; collected: number }[] }) {
          ...
          <div key={f.name}>
            <div className="flex items-center justify-between text-sm mb-1">
              ...
              <span className="text-gray-500 tabular-nums">
                {f.uptake}% · {f.studentsBilled} students · {naira(f.collected)}
              </span>
            </div>
