...
import { startBulkSend } from '@/app/(app)/invoices/actions'
...
export default function BulkSendInvoicesPanel({ count, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    async function run() {
      const start = await startBulkSend()
      if ('error' in start) {
      ...
      trackJob(start.jobId, 'bulk_send', 'Sending invoices', {
        ...
        total: start.total ?? 0,
      }, () => router.refresh(), { href: '/invoices' })
      setJobId(start.jobId)
    }
    run()
  }, [confirmed, trackJob, router])
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] flex flex-col">
        ...
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-base font-semibold text-navy">
            {!confirmed ? 'Send all invoices' : result ? 'Invoices sent' : 'Sending invoices'}
          </h3>
            ...
            <div className="space-y-4">
              <div className="p-4 bg-mint-light/40 border border-mint/30 rounded-xl">
                ...
                <ul className="space-y-1 text-sm text-gray-700">
                  <li><strong className="text-mint">{result.processed}</strong> invoice{result.processed === 1 ? '' : 's'} sent</li>
