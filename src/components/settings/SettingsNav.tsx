...
interface NavItem {
  ...
  icon: string
  comingSoon?: boolean
  // When set, the item only shows if the user has this permission (or is owner).
  perm?: string
  // When set, the item only shows for the owner/super_admin/is_admin — not
  // delegable via any toggle (e.g. export/delete of the whole school's data).
  ownerOnly?: boolean
}
...
interface NavSection {
  title: string
  items: NavItem[]
  // When set, the whole section is hidden unless the user has this permission.
  perm?: string
}
...
export default function SettingsNav() {
  ...
  const canSee = (perm?: string) => !perm || isOwner || permissions.has(perm)

  // Hide sections/items the user has no permission for, then apply the search.
  const permedSections = SECTIONS
    .filter(section => canSee(section.perm))
    .map(section => ({ ...section, items: section.items.filter(item => canSee(item.perm) && (!item.ownerOnly || isOwner)) }))
    .filter(section => section.items.length > 0)

  const term = search.trim().toLowerCase()
  const filteredSections = term
    ? permedSections.map(section => ({
        ...section,
