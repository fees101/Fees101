...
import { composeInviteEmail, composeEmailChangedToNewAddress, composeEmailChangedToOldAddress } from '@/lib/messaging/composeInvite'
...
export async function addStaff(input: AddStaffInput) {
  ...
  // generateLink() creates the auth user the same way inviteUserByEmail() did,
  // but returns the action link instead of emailing it — we send our own
  // FEES101-branded email (composeInviteEmail) through the same Brevo path
  // every other outbound email already uses, instead of Supabase's dashboard-
  // configured "Invite user" template.
  //
  // `next` carries the invitee's email (not their id — the auth user doesn't
  // exist yet at this point, so its id can't be baked in ahead of time) so
  // that if the link expires before they set a password, /set-password can
  // look up who invited them and offer "notify whoever invited you."
  const redirectTo = `${origin}/auth/callback?next=${encodeURIComponent(`/set-password?email=${encodeURIComponent(email)}`)}`
  ...
  await sendEmail(
    { supabase: ctx.supabase, schoolId: ctx.schoolId, messageType: 'invite' },
    email,
    composeInviteEmail({
...
export async function resendInvite(userId: string) {
  ...
  // Re-inviting an existing, still-unconfirmed user regenerates a fresh
  // token via the same generateLink() path as addStaff(), then sends our own
  // FEES101-branded email instead of Supabase's dashboard template.
  const { data: inviteData, error: inviteError } = await admin.auth.admin.generateLink({
    type: 'invite',
    email: staff.email,
    options: {
      redirectTo: `${origin}/auth/callback?next=${encodeURIComponent(`/set-password?email=${encodeURIComponent(staff.email)}`)}`,
  ...
  await sendEmail(
    ...
    composeInviteEmail({
      schoolName: school?.name || '',
      roleName: (staff.roles as any)?.name || '',
      inviterName: actor?.name,
      actionUrl: inviteData.properties.action_link,
    }),
  )
