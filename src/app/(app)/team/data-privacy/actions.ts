...
// Undoes requestAccountDeletion() during the grace window. Called from the
// login screen's "cancel the deletion" card, reached by the owner signing
// back in during the grace period (login/actions.ts's login() keeps their
// session alive for exactly this instead of signing them straight back out).
//
// Deliberately checks ctx.role directly instead of the usual ctx.isOwner
// gate: scheduling the deletion deactivated every login on the school,
// including the owner's own, and ctx.isOwner is always false while
// ctx.isActive is false (see permissions.ts) — so the normal gate would make
// this permanently unreachable by the only person allowed to use it.
export async function cancelScheduledDeletion() {
  ...
  const { data: reqRow } = await svc
    ...
    .eq('status', 'scheduled')
    .maybeSingle()
  if (!reqRow) return { error: 'No scheduled deletion was found for this school.' }

  const { data: me } = await svc.from('users').select('name').eq('id', ctx.userId).maybeSingle()
  const actorName = me?.name || 'Owner'

  const { error: updateErr } = await svc
    .from('school_deletion_requests')
    .update({ status: 'cancelled', cancelled_at: new Date().toISOString(), cancelled_by: actorName })
    .eq('id', reqRow.id)
  if (updateErr) return { error: 'Could not cancel the deletion. Please try again or contact support.' }

  // Undo the mass deactivation requestAccountDeletion() performed.
  await svc.from('users').update({ is_active: true }).eq('school_id', ctx.schoolId)
  ...
  try {
    await svc.from('audit_log').insert({
      ...
      target_type: 'school',
      target_id: ctx.schoolId,
      summary: 'Cancelled the scheduled account closure',
    })
  } catch {
    // ignore
  }

  revalidatePath('/team/data-privacy')
