...
export async function createRole(name: string, description?: string, permissions?: Record<string, boolean>) {
  ...
  // Only persist known keys — same guard saveRolePermissions uses — so a
  // stale client can't seed a role with a made-up key.
  const clean: Record<string, boolean> = {}
  for (const k of PERMISSION_KEYS) clean[k] = permissions?.[k] === true

  const { data: created, error } = await ctx.supabase.from('roles').insert({
  ...
  const grantedCount = Object.values(clean).filter(Boolean).length
  await logAuditEvent(ctx.supabase, {
    schoolId: ctx.schoolId,
    actorId: ctx.userId,
    action: 'role.created',
    targetType: 'role',
    summary: `Created role "${trimmed}" with ${grantedCount} of ${PERMISSION_KEYS.length} permissions granted`,
    metadata: { name: trimmed, description: description?.trim() || null, permissions: clean },
  })

  revalidatePath('/', 'layout')
  return { success: true, roleId: created.id as string }
}
