...
export async function saveReminderSettings(form: {
  ...
  const after = {
    ...
    // SMS can never be turned off — it's the only channel guaranteed to work.
    channels: { sms: true as const, email: form.channels.email, whatsapp: false },
  }
  ...
  await logAuditEvent(supabase, {
    ...
    summary,
    metadata: {
      before, after, enabledChanged, enabled: after.enabled,
      ...(form.reason?.trim() ? { reason: form.reason.trim() } : {}),
    },
  })

  revalidatePath('/school/reminders')
  return { success: true }
}
