...
// touches any existing line (discounts, credit_applied, previous_balance are
// all left exactly as they are). That's what makes it safe to run regardless
...
export async function applyOptInAdditionToLiveInvoice(
  ...
  const { data: invoice } = await supabase
    .from('invoices')
    .select('id, line_items, subtotal, total_amount, paid_amount, outstanding_amount, sent_at')
  ...
  const { error } = await supabase
    ...
    .update({
      ...
      outstanding_amount: Number(invoice.outstanding_amount) + amount,
      status: newStatus,
  ...
  await logAuditEvent(supabase, {
    ...
    metadata: { studentId, feeItemId, feeItemName: feeItem.name, amount, invoiceSentAt: invoice.sent_at },
  })
