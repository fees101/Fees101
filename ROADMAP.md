...
- [~] **SMS provider switch: Termii → Sendchamp** — Sendchamp Sender ID (`Fees101`) approved for transactional/DND use; adapter (`src/lib/messaging/sendchamp.ts`) wired in via `SMS_PROVIDER` (default `sendchamp`, set to `termii` to roll back — kept untouched as fallback). End-to-end live-tested 2026-08-18: found and fixed a real bug where the send request's `to` field was sent as an array, which silently routed every send into Sendchamp's batch/bulk mode — accepted and billed, but with no per-message id/reference. Fixed by sending `to` as a plain string, which now returns a real reference matching Sendchamp's dashboard Transaction Ref format. Added a `webhook_events` audit table (`db/webhook_debug.sql`) plus phone-number fallback matching in the webhook route (`api/webhooks/sendchamp`), since Sendchamp's DLR webhook payload has no reliable id to match against `message_logs` directly. **Open issue, escalated to Sendchamp support:** even after the fix, real sends are accepted/billed but never progress past `status: sent` to `delivered`, and the registered webhook has never once been called — reproduced both through our app and through Sendchamp's own dashboard "Test SMS" tool. Blocked on their response before Sendchamp can fully replace Termii; not switching schools over until delivery is confirmed working.
