...
- [ ] **Dashboard content for narrow-permission users** — right now the dashboard only has financial KPIs and the recent-activity feed, both permission-gated; a staff member with e.g. only `manage-students` sees just their name and a greeting once those are hidden. Add content scoped to what a narrow role can actually do — a students count, quick actions ("add a student", "record an opt-in"), whatever fits the permission — instead of leaving the page empty. At minimum, an explicit empty state ("nothing to show — ask your admin about access") beats a blank page.
