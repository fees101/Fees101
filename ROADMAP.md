...
- [ ] **Fresh schema baseline** — `fees101_schema.sql` is stale (predates Paystack — missing `payments`/`students` provider columns and the `webhook_events`/`processed_provider_transactions` tables, added directly in the Supabase SQL editor). Pull a fresh `pg_dump`/schema export and commit missing table DDL. Two now-dead columns on `students` (`virtual_account_number`, `virtual_account_bank`, superseded by `provider_dva_*`) can be dropped once this baseline exists.
- [ ] **Consolidate db/ into per-table schema files** — currently `db/*.sql` is one file per change/feature (roles_permissions.sql, permission_based_rls.sql, etc.), so there's no single place to see a table's current, final definition — you have to trace across files. After the fresh schema baseline above, reorganize into one file per table (or one master file sectioned by table) reflecting the live definition; keep the per-change files as historical record but stop treating them as the reference.
